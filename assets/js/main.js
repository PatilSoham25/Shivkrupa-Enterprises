...
//         dropdownParent.classList.toggle("open");  // 👈 Parent ko stable height
//         menu.classList.toggle("show");
//     });
// });
...
// add AOS attributes via JS (so we don't touch your HTML)
// add AOS attributes via JS (NOT for 3D model to avoid conflict)
(function () {

    // DO NOT apply AOS to 3D section — your custom animation will work
    // (this is the fix)

    document.querySelectorAll('.about-card').forEach((el) => {
        if (!el.hasAttribute('data-aos')) {
            el.setAttribute('data-aos', 'fade-left');
            el.setAttribute('data-aos-delay', '140');
        }
    });
    ...
    document.querySelectorAll('.stat').forEach((el, i) => {
        if (!el.hasAttribute('data-aos')) {
            ...
            el.setAttribute('data-aos-delay', String(80 + i * 80));
        }
    });
    ...
    document.querySelectorAll('.product-card').forEach((el, i) => {
        if (!el.hasAttribute('data-aos')) {
            ...
            el.setAttribute('data-aos-delay', String(80 + i * 90));
        }
    });

    const testWrap = document.querySelector('.testimonial-wrapper');
    if (testWrap && !testWrap.hasAttribute('data-aos')) {
        testWrap.setAttribute('data-aos', 'fade-up');
        testWrap.setAttribute('data-aos-delay', '120');
    }

    const clientsHead = document.querySelector('.clients-header');
    if (clientsHead && !clientsHead.hasAttribute('data-aos')) {
        clientsHead.setAttribute('data-aos', 'fade-up');
    }
})();

// hero image auto crossfade + label swap
(function () {
    const shots = Array.from(document.querySelectorAll('.hero-shot'));
    const labelEl = document.getElementById('heroStepLabel');
    if (!shots.length || !labelEl) return;
...
// intersection observer: reveal + stat orbit
(function () {
    const observer = new IntersectionObserver((entries) => {
    ...
    const toReveal = document.querySelectorAll(
        '.about-card, .why-card, .product-card, .testimonial-wrapper, .clients-header, .clients-section, .stat'
    );
